...
void HistoryAdd(char* commands[]) {
	...
	// char chararr[COMMAND_LENGTH];
	// sprintf(chararr, "%d", HistCount);
	// write(STDOUT_FILENO, chararr, strlen(chararr));
}


//run commands, copied same code from main function
void RunCommand(char*commands[]){

	char pwdcom[COMMAND_LENGTH];
	_Bool in_background = false;

	if(strcmp(commands[0], "exit") == 0){ 	// exit shell
		exit(0);

	}else if(strcmp(commands[0], "pwd") == 0){ 	// pwd, prints the current directory, using getcwd()
	...
	}else if(strcmp(commands[0], "cd") == 0){ // cd, changes directory using chdir()
		if (chdir(commands[1]) != 0) { //if entered invalid directory
			write(STDOUT_FILENO, "Invalid directory. \n", strlen("Invalid directory. \n"));
		}

	}else if(strcmp(commands[0], "type") == 0 ){ // type, output the type, buitin or external
		if(commands[1] != NULL) { // segfault catch if user only enter "type"
			if(strcmp(commands[1], "exit") == 0 || strcmp(commands[1], "pwd") == 0  || strcmp(commands[1], "cd") == 0  || strcmp(commands[1], "history") == 0 ){
	...
	}else if(strcmp(commands[0], "history") == 0){// history, do history stuff
		Historywrite();
		...
		if(mypid < 0){ //error: fork failed
			...
			write(STDOUT_FILENO, commands[0], strlen(commands[0])); // output for unknown command
...
//check run commands
void CheckRunCommand(char* commands[]){

	int index = 0;

	if(HistCount == 0){ //no history
	...
	if(strcmp(commands[0], "!!") == 0){
		...
		int n = atoi(hold);
		if(n <= 0 || n < HistCount - 9 || n > HistCount){ //catch error: if history is empty or user has an noninteger input or invalid integer input
			write(STDOUT_FILENO, "SHELL: Unknown history command.\n", strlen("SHELL: Unknown history command.\n"));
			return;
		}else{
			if(HistCount <= 10){
	...
	char *newToken[NUM_TOKENS];
	int count = tokenize_command(MyHistoryies[index], newToken);

	for (int i = 0; i < count; i++) { //write newtoken out
		write(STDOUT_FILENO, newToken[i], strlen(newToken[i]));
	...
	HistoryAdd(newToken);
	RunCommand(newToken);
	//HistoryAdd(newToken, in_background);
}
